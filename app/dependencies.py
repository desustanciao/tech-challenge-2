...
async def get_items_crud(db: AsyncSession = Depends(get_db), settings: Settings = Depends(get_settings)) -> ItemsCRUD:
    """
...
async def get_user_crud(db: AsyncSession = Depends(get_db), settings: Settings = Depends(get_settings)) -> UserCRUD:
    """
...
async def get_current_user(token: str = Depends(get_token_from_request), users: UserCRUD = Depends(get_user_crud)):
    """
